...
func TestPublishLocalProviderResourcePreservesIdentityAndPrivateAccess(t *testing.T) {
	for _, failUpload := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			oldTransport := outboundTransport
			...
			t.Cleanup(func() { outboundTransport = oldTransport; transport.CloseIdleConnections() })
			// The transport routes requests to httptest; use a documentation IP so
			// configuration validation does not depend on external example DNS.
			_, err := svc.UpdateUserOSSSetting(&model.User{ID: "owner"}, OSSSettingRequest{Enabled: true, Provider: "aliyun", Endpoint: "http://192.0.2.1", Bucket: "bucket", PathPrefix: "9gtoken-assets/9glab", AccessKeyID: "test-id", AccessKeySecret: "test-secret"})
			if err != nil {
				t.Fatal(err)
			}
			media := providerMedia{StorageKey: "resource:image-1"}
			err = svc.hydrateProviderMedia("owner", &media, true)
			...
			if failUpload {
				...
				if err := svc.hydrateProviderMedia("owner", &providerMedia{StorageKey: "resource:image-1"}, true); err != nil {
					t.Fatal(err)
				}
				if puts != 1 {
			...
			if err := svc.hydrateProviderMedia("other-owner", &providerMedia{StorageKey: "resource:image-1"}, true); err == nil {
