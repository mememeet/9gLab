...
func TestGatewayAssetPreparationRetryContract(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			config := providerConfig{BaseURL: server.URL, InterfaceType: provider, AllowLocalChannel: true}
			ctx, cancel := context.WithCancel(withProviderOutboundPolicy(context.Background(), config))
			defer cancel()
			waits := 0
