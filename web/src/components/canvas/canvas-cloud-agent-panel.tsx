...
export function CanvasCloudAgentPanel({ canvasId, domainProjectId, nodeCount, references, open, prefillPrompt, onOpen, onCollapse, onFocusNode }: CloudAgentPanelProps) {
    ...
    const { selectedModel, selectedModelError } = useMemo(() => {
        try {
            return { selectedModel: resolveAgentTextModel(config), selectedModelError: "" };
        } catch (cause) {
            return { selectedModel: "", selectedModelError: cause instanceof Error ? cause.message : String(cause) };
        }
    }, [config]);
    ...
    const submit = async () => {
        ...
        let accepted = false;
        try {
            if (selectedModelError || !selectedModel) throw new Error(selectedModelError || "请先选择可用的 Agent 文本模型");
    ...
    const setModel = (model: string) => {
        updateConfig("agentTextModel", model);
    };
                        ...
                        <AnimatePresence mode="wait" initial={false}>
                            {view === "settings" ? (
                                ...
                                <motion.div key="chat" className="flex min-h-0 flex-1 flex-col" initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: reducedMotion ? 0 : 0.18 }}>
                                    ...
                                    />
                                    {selectedModelError ? <div role="alert" className="px-5 pb-2 text-xs text-red-500">{selectedModelError}</div> : null}
                                    <AgentChatComposer
                                        prompt={prompt}
                                        disabled={busy || running || Boolean(run && connectionStatus !== "connected") || !historyHydrated || !pendingHydrated || Boolean(selectedModelError)}
