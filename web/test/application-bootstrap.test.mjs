...
async function prepareEntry(dev, pathname) {
    ...
    const window = Object.assign(new EventTarget(), { location: { pathname } });
    runInNewContext(await build.outputs[0].text(), { window, events, appearanceReady, entryLoaded });
...
for (const [dev, pathname] of [
    ...
    test(`appearance loads alongside normal startup: dev=${dev} path=${pathname}`, async () => {
