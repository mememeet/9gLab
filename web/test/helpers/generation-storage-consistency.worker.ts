...
async function runCanvasBatchCommitRace() {
    ...
    apiClient.defaults.adapter = async (config) => {
        if (config.url !== "/resources/access") throw new Error(`Unexpected test request: ${config.url}`);
        const requests = JSON.parse(config.data as string) as Array<{ resourceId: string }>;
        return { config, status: 200, statusText: "OK", headers: {}, data: { code: 0, data: { items: requests.map(({ resourceId }) => ({ resourceId, access: { resourceId, url: `/api/resources/${resourceId}/file`, actualVariant: "original", requestedVariant: "original", delivery: "platform-local" } })) } } };
    };
    const previousScope = getActiveUserScope();
