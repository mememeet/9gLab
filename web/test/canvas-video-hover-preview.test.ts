...
class FakeElement extends EventTarget {
    ...
    closest(selector: string) {
        return selector.includes('="true"') ? (interacting ? this : null) : this;
    }
    getAttribute() {
        return interacting ? "true" : null;
    }
    appendChild(video: FakeVideo) {
        videos.push(video);
    }
}
        ...
        Object.assign(new EventTarget(), {
            matchMedia: () => {
                const media = Object.assign(new EventTarget(), {
                    matches: reduced,
                    addListener: (listener: EventListener) => media.addEventListener("change", listener),
                    removeListener: (listener: EventListener) => media.removeEventListener("change", listener),
                });
                return media;
            },
        }),
...
test("new hover cancels previous decoder and ignores stale URL resolution", async () => {
    ...
    expect(videos[0].src).toBe("test.mp4");
    enter(slow);
...
test("wheel, viewport interaction and leaving viewport cancel playback", async () => {
    ...
    interacting = true;
    mutation();
    expect(videos[1].removed).toBe(true);
    interacting = false;
    enter(element);
    await wait();
    visibility([{ isIntersecting: false }]);
    expect(videos[2].removed).toBe(true);
});
test("manual playback is never competing with hover", async () => {
    const manual = new FakeVideo();
    manual.paused = false;
    videos.push(manual);
    let calls = 0;
    const element = setup(async () => {
