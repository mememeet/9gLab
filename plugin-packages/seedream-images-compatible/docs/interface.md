# Seedream Images 中转兼容 接口字段

## 协议身份

- 插件 ID：`seedream-images-compatible`。
- Provider ID：`seedream-images-compatible`。
- 能力：`image`。
- 默认 Base URL：``。
- 鉴权驱动：`bearer`。
...
用于 9gToken 等支持 Seedream JSON 图片请求的中转服务。渠道填写中转站根地址，模型填写该站公开模型 ID；参考图通过 image 数组传递。火山方舟直连继续使用独立的 volcengine-ark-image 协议。
...
```json
{
  ...
  "id": "seedream-images-compatible",
  "name": "Seedream Images 中转兼容",
  "version": "2.0.0",
  "author": "OpenAI compatible / 影策",
  "description": "Seedream Images 中转兼容 独立请求协议插件。",
